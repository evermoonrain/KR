...
def get_universe(refresh: bool = False) -> List[Stock]:
    ...
    if fetched:
        logger.warning(
            f"동적 조회(네이버)로 {len(fetched)}개 종목 로드 완료 - "
            f"주의: 네이버 API는 시가총액 정렬을 보장하지 않아 실제 "
            f"코스피200/코스닥100 구성종목과 다를 수 있는 추정치입니다. "
            f"정확도를 위해 config/universe_cache.csv를 공식 구성종목으로 직접 갱신하는 것을 권장합니다."
        )
        try:
            save_cache(fetched)
        except Exception as e:

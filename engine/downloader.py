...
def download_universe(stocks: List, period: str = None) -> Dict[str, pd.DataFrame]:
    """
    유니버스 종목들의 일봉 데이터를 안전하게 다운로드합니다.
    기본 기간은 config/settings.py의 PARAMS["price_history_days"] (기본 5년, 월봉 RSI 계산용).
    """
    if period is None:
        period = PARAMS["price_history_days"]
    if not stocks:
        return {}

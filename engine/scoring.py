...
from engine.indicators import compute_all_indicators, monthly_rsi
...
class ScanResult:
    ...
    gated: bool = False   # True면 월봉RSI 게이트 미통과로 9단계 지표 계산을 생략함
...
def evaluate_stock(code: str, name: str, market: str, ticker: str, df: pd.DataFrame) -> ScanResult:
    """
    단일 종목 평가 순서:
    1) 월봉 RSI 게이트 필터 먼저 계산 (원본 df의 종가만 사용, 가벼운 연산)
       - 게이트를 통과하지 못하면 거래량/OBV/VWAP/MFI/MACD/KDJ/RSI/ADX/볼린저 등
         9단계 지표는 아예 계산하지 않고 즉시 탈락 처리한다.
    2) 게이트를 통과한 종목만 compute_all_indicators로 9단계 지표를 계산하고
       가중치 기반 점수/등급을 산정한다.
    """
    close = float(df["Close"].iloc[-1])
    m_rsi = monthly_rsi(df, PARAMS["monthly_rsi_period"])
    gate_max = PARAMS["monthly_rsi_gate_max"]

    if np.isnan(m_rsi) or m_rsi > gate_max:
        # 월봉RSI 게이트 미통과 -> 9단계 지표 계산 생략, 즉시 탈락 처리
        return ScanResult(
            code=code,
            name=name,
            market=market,
...
def filter_and_rank(results: List[ScanResult]) -> List[ScanResult]:
    """
    1) 월봉RSI 게이트 미통과(gated=True) 종목 제외 (evaluate_stock에서 이미 9단계 계산을 생략한 종목)
    2) MIN_PASS_SCORE 미만 탈락
    3) 점수 내림차순 정렬
    """
    passed = [
        r for r in results
        if not r.gated
        and r.total_score >= MIN_PASS_SCORE
    ]

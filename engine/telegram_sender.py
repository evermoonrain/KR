...
def build_summary_message(results: List[ScanResult], scan_date: str) -> str:
    ...
    lines = []
    lines.append(f"📊 *국장스캐너* {_escape_md('[KR] ' + scan_date)}")
    lines.append(f"💻 *대상: KOSPI200 \\+ KOSDAQ100")
    lines.append("")
...
def notify(results: List[ScanResult], excel_path: str, scan_date: str = None):
    ...
    if not results:
        # 통과 종목이 없어도 엑셀 파일은 항상 생성되므로 반드시 함께 전송한다.
        # 엑셀의 [제외종목_RSI확인] 시트에서 월봉RSI/9단계 필터 탈락 사유를 확인할 수 있다.
        msg = (
            f"⚠️ {_escape_md(scan_date)} 스캔 결과 통과 종목이 없습니다\\.\n"
            f"📎 탈락 사유\\(월봉RSI 수치 / 9단계 필터 점수\\)는 첨부 엑셀의 "
            f"'제외종목\\_RSI확인' 시트를 참고하세요\\."
        )
        send_message(msg)
        if excel_path and os.path.exists(excel_path):
            send_document(excel_path, caption=f"OMNI KR STOCK SCAN {scan_date} (통과 종목 없음)")
        else:
            logger.warning("엑셀 파일 경로가 없어 문서 전송을 건너뜁니다.")
        return

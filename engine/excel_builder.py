# -*- coding: utf-8 -*-
"""
Excel 리포트 빌더
- Sheet 1: 요약 (등급별 종목 리스트 + 총점)
- Sheet 2: 상세 점수표 (9단계 필터 단계별 점수/통과여부)
- Sheet 3: 시스템 설명 (가중치표 + 필터 우선순위표)
"""

import logging
import os
from datetime import datetime
from typing import List

from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
from openpyxl.utils import get_column_letter
from openpyxl.formatting.rule import CellIsRule

from config.settings import WEIGHTS, PARAMS, FILTER_PIPELINE, GRADE_CUTOFFS, MIN_PASS_SCORE, OUTPUT_DIR, EXCEL_FILENAME_PREFIX
from engine.scoring import ScanResult, PIPELINE_ORDER

logger = logging.getLogger("excel_builder")

FONT_NAME = "Arial"

GRADE_COLORS = {
    "S": "FFD700",  # gold
    "A": "92D050",  # green
    "B": "FFEB84",  # yellow
    "C": "F4B084",  # orange
}

HEADER_FILL = PatternFill("solid", start_color="305496", end_color="305496")
HEADER_FONT = Font(name=FONT_NAME, bold=True, color="FFFFFF", size=11)
TITLE_FONT = Font(name=FONT_NAME, bold=True, size=16, color="1F3864")
SUB_FONT = Font(name=FONT_NAME, size=10, italic=True, color="666666")

THIN_BORDER = Border(
    left=Side(style="thin", color="D9D9D9"),
    right=Side(style="thin", color="D9D9D9"),
    top=Side(style="thin", color="D9D9D9"),
    bottom=Side(style="thin", color="D9D9D9"),
)


def _style_header_row(ws, row_idx: int, n_cols: int):
    for c in range(1, n_cols + 1):
        cell = ws.cell(row=row_idx, column=c)
        cell.fill = HEADER_FILL
        cell.font = HEADER_FONT
        cell.alignment = Alignment(horizontal="center", vertical="center")
        cell.border = THIN_BORDER


def _autofit(ws, widths: dict):
    for col, width in widths.items():
        ws.column_dimensions[col].width = width


def build_summary_sheet(wb: Workbook, results: List[ScanResult], scan_date: str):
    ws = wb.active
    ws.title = "요약"

    ws["A1"] = "OMNI KR STOCK SCANNER - 종목발굴 리포트"
    ws["A1"].font = TITLE_FONT
    ws.merge_cells("A1:H1")

    ws["A2"] = (
        f"스캔 기준일: {scan_date}   |   대상: KOSPI200 + KOSDAQ100   |   "
        f"탈락 기준: {MIN_PASS_SCORE}점 미만 또는 월봉RSI>{PARAMS['monthly_rsi_gate_max']}"
    )
    ws["A2"].font = SUB_FONT
    ws.merge_cells("A2:H2")

    grade_counts = {g: 0 for g, _ in GRADE_CUTOFFS}
    for r in results:
        if r.grade in grade_counts:
            grade_counts[r.grade] += 1
    summary_line = "  ".join([f"{g}등급 {n}건" for g, n in grade_counts.items()])
    ws["A3"] = f"등급 분포: {summary_line}  (총 {len(results)}건 통과)"
    ws["A3"].font = SUB_FONT
    ws.merge_cells("A3:H3")

    if len(results) == 0:
        ws["A4"] = "⚠️ 이번 스캔에서 통과 종목이 없습니다. 탈락 사유(월봉RSI 수치 / 9단계 점수)는 [제외종목_RSI확인] 시트를 참고하세요."
        ws["A4"].font = Font(name=FONT_NAME, bold=True, color="CC0000", size=11)
        ws.merge_cells("A4:H4")

    header_row = 5
    headers = ["순위", "종목코드", "종목명", "시장", "현재가", "총점", "등급", "월봉RSI",
               "거래량", "OBV", "VWAP", "MFI", "MACD", "KDJ", "RSI", "ADX", "볼린저"]
    for i, h in enumerate(headers, start=1):
        ws.cell(row=header_row, column=i, value=h)
    _style_header_row(ws, header_row, len(headers))

    key_to_col = {key: 9 + idx for idx, key in enumerate(PIPELINE_ORDER)}  # I=9부터

    for rank, r in enumerate(results, start=1):
        row = header_row + rank
        ws.cell(row=row, column=1, value=rank)
        ws.cell(row=row, column=2, value=r.code)
        ws.cell(row=row, column=3, value=r.name)
        ws.cell(row=row, column=4, value="코스피200" if r.market == "KOSPI200" else "코스닥100")
        ws.cell(row=row, column=5, value=r.close).number_format = "#,##0"
        ws.cell(row=row, column=6, value=r.total_score).number_format = "0.0"

        grade_cell = ws.cell(row=row, column=7, value=r.grade)
        grade_cell.font = Font(name=FONT_NAME, bold=True)
        grade_cell.fill = PatternFill("solid", start_color=GRADE_COLORS.get(r.grade, "FFFFFF"))
        grade_cell.alignment = Alignment(horizontal="center")

        ws.cell(row=row, column=8, value=round(r.monthly_rsi, 1) if r.monthly_rsi == r.monthly_rsi else None).number_format = "0.0"

        step_map = r.step_dict()
        for key, col in key_to_col.items():
            step = step_map[key]
            cell = ws.cell(row=row, column=col, value=round(step.weighted_score, 1))
            cell.number_format = "0.0"
            if step.passed:
                cell.font = Font(name=FONT_NAME, color="1A7A1A")
            else:
                cell.font = Font(name=FONT_NAME, color="999999")

        for c in range(1, len(headers) + 1):
            ws.cell(row=row, column=c).border = THIN_BORDER

    last_row = header_row + len(results)
    if len(results) > 0:
        score_col = "F"
        rng = f"{score_col}{header_row+1}:{score_col}{last_row}"
        ws.conditional_formatting.add(
            rng,
            CellIsRule(operator="greaterThanOrEqual", formula=["85"], fill=PatternFill("solid", start_color="FFD700")),
        )

    widths = {
        "A": 6, "B": 10, "C": 18, "D": 11, "E": 12, "F": 8, "G": 6, "H": 8,
        "I": 9, "J": 8, "K": 9, "L": 7, "M": 8, "N": 7, "O": 7, "P": 7, "Q": 8,
    }
    _autofit(ws, widths)
    ws.freeze_panes = f"A{header_row+1}"


def build_excluded_sheet(wb: Workbook, all_results: List[ScanResult], ranked: List[ScanResult], scan_date: str):
    """
    탈락(제외) 종목 상세 시트.
    - 섹션 1: 월봉RSI 게이트 미통과 종목 (9단계 지표 계산 자체가 생략된 종목) -> 월봉RSI 수치 확인용
    - 섹션 2: 게이트는 통과했지만 총점이 MIN_PASS_SCORE 미만이라 탈락한 종목 -> 9단계 필터 상세 점수 확인용
    통과 종목이 0건이어도 이 시트는 항상 생성되어, "왜 통과 종목이 없는지"를 바로 확인할 수 있게 한다.
    """
    ws = wb.create_sheet("제외종목_RSI확인")

    ws["A1"] = "탈락(제외) 종목 상세 - 월봉RSI 게이트 & 9단계 필터 점수 확인"
    ws["A1"].font = TITLE_FONT
    ws.merge_cells("A1:J1")

    gated = [r for r in all_results if r.gated]
    score_failed = [r for r in all_results if not r.gated and r.total_score < MIN_PASS_SCORE]

    ws["A2"] = (
        f"스캔 기준일: {scan_date}   |   전체 평가 종목 {len(all_results)}건 중 "
        f"최종 통과 {len(ranked)}건 / 탈락 {len(gated) + len(score_failed)}건 "
        f"(월봉RSI게이트 미통과 {len(gated)}건 + 점수미달 {len(score_failed)}건)"
    )
    ws["A2"].font = SUB_FONT
    ws.merge_cells("A2:J2")

    row = 4

    # ------------------------------------------------------------
    # 섹션 1: 월봉RSI 게이트 미통과 종목 (RSI 수치 때문에 제외된 종목)
    # ------------------------------------------------------------
    ws.cell(row=row, column=1,
            value=f"■ 섹션 1: 월봉RSI 게이트 미통과 종목 ({len(gated)}건) "
                  f"— 월봉RSI가 게이트 기준({PARAMS['monthly_rsi_gate_max']}) 초과 또는 데이터 부족 → 9단계 지표 계산 자체를 생략하고 즉시 탈락")
    ws.cell(row=row, column=1).font = Font(name=FONT_NAME, bold=True, size=12, color="CC0000")
    ws.merge_cells(f"A{row}:J{row}")
    row += 1

    gate_headers = ["순번", "종목코드", "종목명", "시장", "현재가", "월봉RSI", "게이트기준(이하 통과)", "기준초과폭", "탈락사유"]
    for i, h in enumerate(gate_headers, start=1):
        ws.cell(row=row, column=i, value=h)
    _style_header_row(ws, row, len(gate_headers))
    row += 1

    gate_max = PARAMS["monthly_rsi_gate_max"]
    gated_sorted = sorted(
        gated,
        key=lambda r: (r.monthly_rsi if r.monthly_rsi == r.monthly_rsi else -1),
        reverse=True,
    )
    for idx, r in enumerate(gated_sorted, start=1):
        has_rsi = r.monthly_rsi == r.monthly_rsi  # NaN 체크
        ws.cell(row=row, column=1, value=idx)
        ws.cell(row=row, column=2, value=r.code)
        ws.cell(row=row, column=3, value=r.name)
        ws.cell(row=row, column=4, value="코스피200" if r.market == "KOSPI200" else "코스닥100")
        ws.cell(row=row, column=5, value=r.close).number_format = "#,##0"
        rsi_cell = ws.cell(row=row, column=6, value=round(r.monthly_rsi, 1) if has_rsi else "데이터부족")
        if has_rsi:
            rsi_cell.number_format = "0.0"
        ws.cell(row=row, column=7, value=gate_max)
        gap_cell = ws.cell(row=row, column=8, value=round(r.monthly_rsi - gate_max, 1) if has_rsi else None)
        if has_rsi:
            gap_cell.number_format = "+0.0;-0.0"
        reason = (
            f"월봉RSI {r.monthly_rsi:.1f} > 게이트기준 {gate_max}"
            if has_rsi else "월봉 데이터 부족 (상장 이력 짧음 등)으로 RSI 산출 불가"
        )
        ws.cell(row=row, column=9, value=reason)
        for c in range(1, len(gate_headers) + 1):
            ws.cell(row=row, column=c).border = THIN_BORDER
        row += 1

    if not gated_sorted:
        ws.cell(row=row, column=1, value="(해당 없음 - 게이트 미통과 종목 없음)")
        ws.cell(row=row, column=1).font = SUB_FONT
        row += 1

    row += 2

    # ------------------------------------------------------------
    # 섹션 2: 게이트는 통과했으나 9단계 필터 총점 미달로 탈락한 종목
    # ------------------------------------------------------------
    ws.cell(row=row, column=1,
            value=f"■ 섹션 2: 게이트 통과, 총점 미달 탈락 종목 ({len(score_failed)}건) "
                  f"— 9단계 필터는 모두 계산됨. 총점 < {MIN_PASS_SCORE}점 기준 미달")
    ws.cell(row=row, column=1).font = Font(name=FONT_NAME, bold=True, size=12, color="CC0000")
    ws.merge_cells(f"A{row}:J{row}")
    row += 1

    score_headers = ["순번", "종목코드", "종목명", "시장", "현재가", "월봉RSI", "총점", "부족점수", "등급",
                      "거래량", "OBV", "VWAP", "MFI", "MACD", "KDJ", "RSI", "ADX", "볼린저"]
    for i, h in enumerate(score_headers, start=1):
        ws.cell(row=row, column=i, value=h)
    _style_header_row(ws, row, len(score_headers))
    row += 1

    key_to_col = {key: 10 + idx for idx, key in enumerate(PIPELINE_ORDER)}  # J=10부터

    score_failed_sorted = sorted(score_failed, key=lambda r: r.total_score, reverse=True)
    for idx, r in enumerate(score_failed_sorted, start=1):
        ws.cell(row=row, column=1, value=idx)
        ws.cell(row=row, column=2, value=r.code)
        ws.cell(row=row, column=3, value=r.name)
        ws.cell(row=row, column=4, value="코스피200" if r.market == "KOSPI200" else "코스닥100")
        ws.cell(row=row, column=5, value=r.close).number_format = "#,##0"
        ws.cell(row=row, column=6, value=round(r.monthly_rsi, 1) if r.monthly_rsi == r.monthly_rsi else None).number_format = "0.0"
        ws.cell(row=row, column=7, value=r.total_score).number_format = "0.0"
        ws.cell(row=row, column=8, value=round(MIN_PASS_SCORE - r.total_score, 1)).number_format = "0.0"
        ws.cell(row=row, column=9, value=r.grade)

        step_map = r.step_dict()
        for key, col in key_to_col.items():
            step = step_map.get(key)
            if step is None:
                continue
            cell = ws.cell(row=row, column=col, value=round(step.weighted_score, 1))
            cell.number_format = "0.0"
            cell.font = Font(name=FONT_NAME, color="1A7A1A" if step.passed else "999999")

        for c in range(1, len(score_headers) + 1):
            ws.cell(row=row, column=c).border = THIN_BORDER
        row += 1

    if not score_failed_sorted:
        ws.cell(row=row, column=1, value="(해당 없음 - 점수미달 탈락 종목 없음)")
        ws.cell(row=row, column=1).font = SUB_FONT
        row += 1

    widths = {
        "A": 6, "B": 10, "C": 18, "D": 11, "E": 12, "F": 9, "G": 14, "H": 10,
        "I": 8, "J": 8, "K": 8, "L": 8, "M": 7, "N": 8, "O": 7, "P": 7, "Q": 7, "R": 8,
    }
    _autofit(ws, widths)
    ws.freeze_panes = "A5"


def build_detail_sheet(wb: Workbook, results: List[ScanResult]):
    ws = wb.create_sheet("상세점수표")

    ws["A1"] = "단계별 상세 점수 및 판정 근거"
    ws["A1"].font = TITLE_FONT
    ws.merge_cells("A1:F1")

    row = 3
    name_map = {f["key"]: f["name"] for f in FILTER_PIPELINE}

    for r in results:
        ws.cell(row=row, column=1, value=f"[{r.grade}] {r.code} {r.name} (총점 {r.total_score})")
        ws.cell(row=row, column=1).font = Font(name=FONT_NAME, bold=True, size=12)
        ws.merge_cells(f"A{row}:F{row}")
        row += 1

        sub_header = ["단계", "지표", "가중치", "통과여부", "단계점수", "상세"]
        for i, h in enumerate(sub_header, start=1):
            ws.cell(row=row, column=i, value=h)
        _style_header_row(ws, row, len(sub_header))
        row += 1

        for step_idx, key in enumerate(PIPELINE_ORDER, start=1):
            step = r.step_dict()[key]
            ws.cell(row=row, column=1, value=step_idx)
            ws.cell(row=row, column=2, value=name_map[key])
            ws.cell(row=row, column=3, value=WEIGHTS[key])
            pass_cell = ws.cell(row=row, column=4, value="통과" if step.passed else "미통과")
            pass_cell.font = Font(name=FONT_NAME, color="1A7A1A" if step.passed else "CC0000", bold=True)
            ws.cell(row=row, column=5, value=round(step.weighted_score, 2)).number_format = "0.00"
            ws.cell(row=row, column=6, value=step.detail)
            for c in range(1, 7):
                ws.cell(row=row, column=c).border = THIN_BORDER
            row += 1

        row += 1  # 종목간 간격

    widths = {"A": 6, "B": 14, "C": 8, "D": 10, "E": 10, "F": 40}
    _autofit(ws, widths)


def build_system_sheet(wb: Workbook):
    ws = wb.create_sheet("시스템설명")

    ws["A1"] = "스코어링 시스템 구조"
    ws["A1"].font = TITLE_FONT
    ws.merge_cells("A1:D1")

    ws["A3"] = "■ 지표별 가중치"
    ws["A3"].font = Font(name=FONT_NAME, bold=True, size=12)
    row = 4
    ws.cell(row=row, column=1, value="지표")
    ws.cell(row=row, column=2, value="가중치(%)")
    _style_header_row(ws, row, 2)
    row += 1
    weight_label = {
        "volume": "거래량", "obv": "OBV", "vwap": "VWAP", "kdj": "KDJ",
        "macd": "MACD", "mfi": "MFI", "rsi": "RSI", "adx": "ADX", "bb": "볼린저밴드",
    }
    for key in PIPELINE_ORDER:
        ws.cell(row=row, column=1, value=weight_label[key])
        ws.cell(row=row, column=2, value=WEIGHTS[key])
        for c in (1, 2):
            ws.cell(row=row, column=c).border = THIN_BORDER
        row += 1

    row += 2
    ws.cell(row=row, column=1, value="■ 필터 우선순위 파이프라인").font = Font(name=FONT_NAME, bold=True, size=12)
    row += 1
    header = ["단계", "필터", "목적", "중요도"]
    for i, h in enumerate(header, start=1):
        ws.cell(row=row, column=i, value=h)
    _style_header_row(ws, row, 4)
    row += 1
    for idx, f in enumerate(FILTER_PIPELINE, start=1):
        ws.cell(row=row, column=1, value=idx)
        ws.cell(row=row, column=2, value=f["name"])
        ws.cell(row=row, column=3, value=f["purpose"])
        ws.cell(row=row, column=4, value="★" * f["stars"])
        for c in range(1, 5):
            ws.cell(row=row, column=c).border = THIN_BORDER
        row += 1

    row += 2
    ws.cell(row=row, column=1, value="■ 등급 컷오프").font = Font(name=FONT_NAME, bold=True, size=12)
    row += 1
    ws.cell(row=row, column=1, value="등급")
    ws.cell(row=row, column=2, value="최소점수")
    _style_header_row(ws, row, 2)
    row += 1
    for grade, cutoff in GRADE_CUTOFFS:
        ws.cell(row=row, column=1, value=grade).fill = PatternFill("solid", start_color=GRADE_COLORS.get(grade, "FFFFFF"))
        ws.cell(row=row, column=2, value=cutoff)
        for c in (1, 2):
            ws.cell(row=row, column=c).border = THIN_BORDER
        row += 1
    ws.cell(row=row, column=1, value="탈락")
    ws.cell(row=row, column=2, value=f"< {MIN_PASS_SCORE}")
    row += 1

    widths = {"A": 14, "B": 16, "C": 22, "D": 12}
    _autofit(ws, widths)


def build_excel_report(all_results: List[ScanResult], ranked: List[ScanResult] = None, scan_date: str = None) -> str:
    """
    all_results: 유니버스 전체 평가 결과 (게이트 미통과 + 점수미달 + 통과 종목 모두 포함)
    ranked:      필터를 모두 통과한 최종 종목 (없으면 all_results에서 자동 계산)
    통과 종목이 0건이어도 항상 엑셀 파일을 생성하며, 이 경우 [제외종목_RSI확인] 시트에서
    코스피200/코스닥100 각 종목의 월봉RSI 수치 및 9단계 필터 점수를 확인할 수 있다.
    """
    if scan_date is None:
        scan_date = datetime.now().strftime("%Y-%m-%d %H:%M KST")

    if ranked is None:
        # 하위 호환: ranked를 안 넘겨주면 all_results 자체를 통과 목록으로 간주하지 않고
        # gated=False이며 스코어 계산이 끝난 종목 중 필터 재적용
        from engine.scoring import filter_and_rank
        ranked = filter_and_rank(all_results)

    wb = Workbook()
    build_summary_sheet(wb, ranked, scan_date)
    build_excluded_sheet(wb, all_results, ranked, scan_date)
    build_detail_sheet(wb, ranked)
    build_system_sheet(wb)

    os.makedirs(OUTPUT_DIR, exist_ok=True)
    fname = f"{EXCEL_FILENAME_PREFIX}_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx"
    path = os.path.join(OUTPUT_DIR, fname)
    wb.save(path)
    logger.info(f"엑셀 리포트 저장: {path} (통과 {len(ranked)}건 / 전체평가 {len(all_results)}건)")
    return path

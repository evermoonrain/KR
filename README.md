...
9단계 점수화와 별도로, **월봉 RSI ≤ 30**인 종목만 최종 리포트에 남도록 게이트 필터를 적용합니다.
(점수에는 반영되지 않고, 통과/탈락만 결정합니다. 기준값은 `config/settings.py`의
`PARAMS["monthly_rsi_gate_max"]`에서 조정 가능)

월봉 RSI(14)를 신뢰성 있게 계산하려면 최소 14개월 이상의 일봉 데이터가 필요하므로,
`price_history_days`를 기존 180일에서 **5년(`"5y"`)**으로 확장했습니다. 다운로드 시간이 늘어날 수 있습니다.

거래량 급증 기준(`volume_surge_ratio`)은 기존 1.5배에서 **1.2배**로 완화했습니다.
...
**운영 시에는 `universe_cache.csv`를 코스피200/코스닥100 공식 구성종목으로 직접 관리하는 것을 기본 방침으로 합니다.**
네이버 금융 API 보조조회는 시가총액 정렬을 보장하지 않아 실제 구성종목과 다를 수 있는 추정치이며,
캐시가 없을 때만 보조 수단으로 사용됩니다(사용 시 로그에 경고가 남습니다).
